...
## 后续：improved+trailing 增强（2026-06-15，用户要求"improved 回测更强"）

> 先用参数网格区分「可辩护提升」与「过拟合冲高」，再 TDD 落地可辩护的那条。

- **过拟合反例（已否决）**：固定止盈调到 50% → improved +51%，但坐实为 3 笔超热门股（暗盘 +166%~210%）恰好在 50% 止盈位驱动、9 笔全止损（其余 11 笔合计 +1.2%）——look-back 拟合已知赢家，断崖非单调，否决。
- **采纳：追踪止损**。止损线=移动高点×(1−trail)、只上移；进入当日用截至昨日高点判触发（无 look-ahead）；跳空按 open。新增 `improved_trailing_stop`（trail=0.10、持仓放宽到 10 日）。
- **结果**：improved+trailing 12 笔 **+112.6%**、pf 2.70、win 41.7%。trail 扫描 {0.08→+163%, 0.10→+113%, 0.15→+97%, 0.20→+1.6%} **单调下降** = 机制性而非尖峰，比固定止盈调参可辩护。
- **诚实结论**：选股因子（暗盘）经 data-snooping 仍不显著；trailing 高收益建立在 2026 上半年普涨、"让趋势跑"天然占优之上，外推到下跌/震荡市存疑，需多市场样本外验证。data-snooping 的 n_trials 已纳入 trail sweep（=14，更保守）。
- TDD：+3 引擎测试（回撤触发/让赢家跑/无 look-ahead），全套 **59 passed**，幂等。
